...
CHANGES FROM v4.0:
  [v5-1] Human-like typing    - Text fields typed char-by-char (20-50ms/key).
                                 1-in-10 chance of a typo + backspace correction.
                                 Applied only to long fields (EWB, place, vehicle).
                                 Short fields (pincode, distance) still use fast js_fill.
  [v5-2] Random delays        - Between field fills: 0.4-1.2s
                                 Before button clicks: 0.3-0.8s
                                 After page load:      1.5-2.5s
  [v5-3] Random scroll        - 50% chance of a single gentle scroll before filling starts.
                                 Fast, non-disruptive.
  [v5-4] Excel per-bill data  - current_place, current_pincode, approx_distance
                                 are now read from Excel columns per bill.
                                 Falls back to DEFAULT_* values if columns are missing.
"""
...
# ─────────────────────────────────────────────
LICENSE_SHEET_URL = (
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vToI_yucRUI_mE19ruBCjLrZ7HwGEQogKdNICo5Bq6gDsEIZ5ET0zdss_K7YPCZbjt8cZlUaUS-t8Sk/pub?output=csv"
)
...
def human_type(driver, el, text):
    ...
    for ch in text:
        ...
        if random.randint(1, 10) == 1:
            # Type a wrong adjacent key then backspace
        ...
        el.send_keys(ch)
        time.sleep(random.uniform(0.02, 0.05))   # 20–50 ms per key

    # Trigger change events after typing
    driver.execute_script(
        """
        arguments[0].dispatchEvent(new Event('input',  {bubbles: true}));
        arguments[0].dispatchEvent(new Event('change', {bubbles: true}));
...
def do_login(driver):
    ...
    time.sleep(5)   # CSRF token load wait (kept from v4)
...
def extend_one(driver, bill, num, total, debug_mode):
    ...
    try:
        ...
        # ── FIX 1: Alert popup check after Go ─────────────────
        time.sleep(2)
        try:
        ...
        try:
            ...
            if pin_el and pin_el.is_displayed():
                js_fill(driver, pin_el, pin)   # fast fill for short numeric field
...
def main():
    ...
    try:
        ...
        for i, bill in enumerate(bills, 1):
            ...
            save_report(results)   # save after every bill
